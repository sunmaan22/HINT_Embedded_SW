...
외부 인터럽트로 버튼을 받는다
필요한 파일이 모두 이 폴더에 있고, `d2am.atsln` 이 이 폴더의 솔루션이다.

## 실습마다 프로젝트에 넣을 파일

| 답안 | 주제 | 프로젝트에 추가할 파일 |
|---|---|---|
| `homework1.c` | 폴링과 인터럽트 비교 | `homework1.c` · `btn.c` · `led.c` · `btn.h` · `led.h` · `board.h` |
| `homework2.c` | 버튼 네 개를 동시에 처리 | `homework2.c` · `btn.c` · `led.c` · `btn.h` · `led.h` · `board.h` |
...
```
avr-gcc -mmcu=atmega128 -DF_CPU=14745600UL -Os -o homework1.elf homework1.c btn.c led.c
