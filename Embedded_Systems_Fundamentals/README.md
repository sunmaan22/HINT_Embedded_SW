...
| 구분 | 내용 |
|---|---|
| 과정 | 현대NGV K뉴딜 아카데미 · 현대자동차 임베디드 AI 과정 HW 트랙 |
| 자료 | 박규칠 「임베디드시스템의이해」 및 「임베디드 시스템의 이해 추가자료」 |
| 교육 구성 | 2일 16시간, 본문 16개 장과 부록 |
| 주요 대상 | ATmega128 및 교육용 보드 |
| 개발 환경 사례 | Microchip Studio 7, AVR 도구 체인, AVRISP MKII, 직렬 터미널 |
| 본자료 | 441쪽, 시각적 구조와 시연 중심 |
| 추가자료 | 124쪽, 개념 설명과 본문·부록 연결 중심 |
| 정리 범위 | 두 자료를 통합한 학습 노트이며 실습 코드·보드 검증 결과를 뜻하지 않는다. |

### 학습 흐름과 추가자료 페이지

페이지는 **추가자료 PDF의 파일 페이지** 기준이다. 본자료 슬라이드 페이지와 구분한다.
...
추가자료 부록 A는 당시 설치 화면을 설명한다. 설치 절차 자체보다 프로젝트 대상, 빌드 결과, 프로그래머와 실행 중 통신 경로를 구분하는 것이 이 노트의 범위이다.
...
추가자료 부록 C의 실제 번호인 **9개 시연**을 기준으로 한다.

| 시연 | 주제 | 연결되는 개념 |
|---|---|---|
| 01 | 빌드·기록·실행 | PC의 파일과 MCU 프로그램 메모리 구분 |
| 02 | LED 패턴 | 8비트 값과 포트 출력 |
| 03 | 스위치로 패턴 변경 | 입력값·프로그램 상태·출력의 연결 |
| 04 | ADC | 전압과 디지털 코드 |
| 05 | CDS 입력 | 빛에 따른 센서 변화와 표시 |
| 06 | PWM | 타이머가 만드는 반복 파형 |
| 07 | 폴링·인터럽트 비교 | 사건 포착과 CPU 실행 흐름 |
| 08 | UART | 내부 데이터를 PC에 전달 |
| 09 | I²C 센서와 UART | 센서 통신과 상태 출력의 통합 |

자료에는 이전 번호가 남은 영상·HEX 이름과 본문 번호의 불일치가 있으므로 파일명만으로 시연 순서를 판단하지 않는다. 특히 **09번 I²C 시연은 자료에서 동작하지 않는 개념 확인용 코드라고 명시**한다. 이 README는 시연의 목적을 설명하며 원문 코드·영상·HEX를 제공하거나 실행 검증을 완료했다는 의미가 아니다.
...
- 사용자 제공 「임베디드시스템의이해_박규칠.pdf」, 441쪽.
- 사용자 제공 「임베디드 시스템의 이해_박규칠 추가자료.pdf」, 124쪽.
- 본문 구성과 세부 설명은 추가자료의 16개 장·부록을 기준으로 하고, 본자료의 도식·보드·시연 흐름을 함께 대조하였다.
- 원문의 문장·그림·코드를 복제하지 않고 핵심 개념을 재서술하였다. 주소 표현, 메모리 배치, 클럭과 데모 현황처럼 혼동하기 쉬운 부분은 적용 조건을 구분하였다.
