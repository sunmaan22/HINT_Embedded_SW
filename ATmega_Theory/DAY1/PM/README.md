...
app / driver / register 3계층을 만들어 본다
필요한 파일이 모두 이 폴더에 있고, `d1pm.atsln` 이 이 폴더의 솔루션이다.

## 실습마다 프로젝트에 넣을 파일

| 답안 | 주제 | 프로젝트에 추가할 파일 |
|---|---|---|
| `homework1.c` | LED 패턴 만들기 (드라이버만 사용) | `homework1.c` · `led.c` · `led.h` · `board.h` |
| `homework2.c` | 버튼으로 모드 전환 | `homework2.c` · `btn.c` · `led.c` · `btn.h` · `led.h` · `board.h` |
| `homework3.c` | FND 에 숫자 띄우기 | `homework3.c` · `fnd.c` · `fnd.h` · `board.h` |
...
```
avr-gcc -mmcu=atmega128 -DF_CPU=14745600UL -Os -o homework1.elf homework1.c led.c
avr-gcc -mmcu=atmega128 -DF_CPU=14745600UL -Os -o homework2.elf homework2.c btn.c led.c
avr-gcc -mmcu=atmega128 -DF_CPU=14745600UL -Os -o homework3.elf homework3.c fnd.c
```
